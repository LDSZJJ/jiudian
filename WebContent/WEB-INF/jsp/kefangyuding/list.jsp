 ...
 $('#duixiang').modal('hide');
   ...
   function addfunction(){
     ...
     if(classone == "active"){
        if(one == ""){
          ...
          parent.document.getElementById('Mainid').src='${ctx}/kefangyuding/toadd.do?id='+one+
       ...
       if(two == "" ){
          ...
          parent.document.getElementById('Mainid').src='${ctx}/kefangyuding/toadd.do?id='+two+
   ...
   var chk_value=[];
  	...
  	if(chk_value!=""){
		if(chk_value.toString().indexOf(",")>0){
		   ...
		   parent.document.getElementById("Mainid").src='${ctx}/kefangyuding/toupdate.do?id='+chk_value;
		}
	}else{
	  alert("请选择一条数据进行修改");
	}
   ...
   function deletefunction(){
    var chk_value=[];
     ...
     if(flag){
  	  parent.document.getElementById("Mainid").src='${ctx}/kefangyuding/delete.do?id='+chk_value;
  	}
  	}else{
  ...
  function souSuo(){
     ...
     $.ajax({      
         cache:false,
         type: "POST",
         url: '${ctx}/kefangyuding/selectPassenger.do',
         data:"txtname="+name,
         async:false,
   ...
   function confirmfunction(){
    ...
    var paperNumber="";
      ...
      $.ajax({      
         cache:false,
         type: "POST",
         url: '${ctx}/kefangyuding/confirmPassenger.do',
         data:"id="+chk_value,
         async:false,
         success: function (result) {
   ...
   function selectTarget(){
     ...
     $.ajax({      
         cache:false,
         type: "POST",
         url: '${ctx}/kefangyuding/selectTarget.do',
         data:"txtname="+name,
         async:false,
         success: function (result) { 
            for (var key in result) { 
   ...
   function selectOne(){
     ...
     parent.document.getElementById("Mainid").src='${ctx}/kefangyuding/tolist.do?txtname='
     +txtname+'&state='+state;
   }
   
   function selectTwo(){
     var state=document.getElementById("stateId").value;
     parent.document.getElementById("Mainid").src='${ctx}/kefangyuding/tolist.do?state='+state;
   }
   ...
   function arrangeRoom(){
  	...
  	var flag=window.confirm("注意：房间已安排成功，是否转到，住宿登记界面，便于登记旅客信息");
	     if(flag){
	  	  parent.document.getElementById("Mainid").src='${ctx}/kefangyuding/arrangeRoom.do?id='
  ...
  $(".tcdPageCode").createPage({
     ...
     var txtname=document.getElementById("txtnameid").value;
     var state=document.getElementById("stateId").value;
     location.href="${ctx}/kefangyuding/tolist.do?currentPage="+p+"&txtname="+txtname+"&state="+state;
     }
   });
